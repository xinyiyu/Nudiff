...
"""
import sys
sys.path.append(os.path.join(nudiff.__path__[0], 'hover_net')) # add the path of hover_net
import cv2
...
from hovernet_config import Config
...
class TrainManager(Config):
        ...
        for net_name, net_info in net_info_opt.items():
            ...
            if pretrained_path is not None:
                if pretrained_path == -1:
                    ...
                    print(f'last pretrained path: {pretrained_path}')
    ...
    def run(self):
        ...
        shutil.copy2('config.py', self.log_dir) # Change this

        if os.path.exists(f'{self.log_dir}/00/net_epoch=50.tar'):
            print(f"{self.log_dir}/00/net_epoch=50.tar exists, start from phase 1...")
            prev_save_path = self.log_dir + '/00/'
            save_path = self.log_dir + '/01/'
            phase_idx = 1
            phase_info = phase_list[phase_idx]
            self.run_once(
                    phase_info, engine_opt, save_path, prev_log_dir=prev_save_path
                )
        else:
            print('start from phase 0...')
            prev_save_path = None
