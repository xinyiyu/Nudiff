...
class TrainManager(Config):
    ...
    def _get_datagen(self, batch_size, run_mode, target_gen, nr_procs=0, fold_idx=0):
        ...
        # if run_model == 'train':
        #     with open(self.train_path_file, 'r') as f:
        #         file_list = f.readlines()
        # else:
        #     with open(self.valid_path_file, 'r') as f:
        #         file_list = f.readlines()
        # file_list = [x.strip('\n') for x in file_list]
    ...
    def run_once(self, opt, run_engine_opt, log_dir, prev_log_dir=None, fold_idx=0):
        ...
        def get_last_chkpt_path(prev_phase_dir, net_name):
            ...
            epoch_list = [int(v.split('.')[0].split('=')[-1]) for v in os.listdir(prev_phase_dir) if '.tar' in v]
            print(epoch_list)
            ...
            print('last_chkpts_path:', last_chkpts_path)
    ...
    def run(self):
        ...
        shutil.copy2('config.py', self.log_dir)
